...
describe('ProjectDetailsContainer component', () => {
  ...
  test('data is shown after mount API call', async () => {
    ...
    await wait(() => {
      expect(apiProjectGet).toHaveBeenCalledTimes(1);
      expect(getByText('entities.project.projectName')).toBeInTheDocument();
      expect(getByText('entities.project.subscriptionVersion')).toBeInTheDocument();
      expect(getByText('entities.project.subscriptionStartDate')).toBeInTheDocument();
      expect(getByText('entities.project.subscriptionEndDate')).toBeInTheDocument();
